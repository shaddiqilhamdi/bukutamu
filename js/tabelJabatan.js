$(document).ready(function () {
	...
	$('#tabelJabatan').DataTable({
		ajax: {
			url: 'https://script.google.com/macros/s/AKfycbyzIE2e6J1KXWu-Uzs_1yx0JtmasuTXlir_8yerXn-dl13ZqEagoOxzpB2ZvB5X09zP1w/exec?type=jabatan', // Replace with your actual Google Apps Script URL
	...
	$('#tabelJabatan').on('click', '.edit-btn', function () {
		var id = $(this).data('id');
	...
	$('#tabelJabatan').on('click', '.delete-btn', function () {
		var id = $(this).data('id');
	...
	$('#refreshDataBtn').on('click', function () {
		$('#tabelJabatan').DataTable().ajax.reload();
	});
});